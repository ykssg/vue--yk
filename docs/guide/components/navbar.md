...
::: warning ⚠️ 免责声明
本组件（导航栏）仅用于展示公开导航数据，`api-url` 接口地址为明文传递。该接口属于**低风险只读资源**，不涉及用户隐私或敏感操作，暴露地址在前端代码中可见属于正常现象，不构成安全漏洞。
...
通过 `api-url` 传入接口地址，组件 `onMounted` 时自动 fetch 获取导航数据，适合动态配置场景。接口可返回数组，或 `{ list: [...] }`、`{ data: [...] }` 结构。

<div class="navbar-demo">
  <yk-navbar api-url="https://api.example.com/navbar/list" :height="48" />
</div>
...
通过 `height` 设置整体高度，数字自动补 `px`，也可传字符串如 `"3rem"`。

<div class="navbar-demo">
  <yk-navbar :navList="[{ id: 'home', name: '首页', image: '/images/home.png' }]" :height="56" />
</div>
...
| `height` | `number \| string` | `''` | 整体高度，数字补 `px` |
